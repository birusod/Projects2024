...
orcas |> 
  ...
  mutate(observers = fct_lump_n(observers, n = 10) |> 
           fct_infreq() |> fct_rev()) |> 
  ggplot(aes(observers)) +
  geom_bar() +
  coord_flip()
...
orcas |> 
  filter(!is.na(vessel)) |> 
  mutate(vessel = fct_lump_n(vessel, n = 10) |> 
           fct_infreq() |> fct_rev()) |> 
  ggplot(aes(vessel)) +
  geom_bar() +
  coord_flip()
...
ggsave(
  filename = file.path("W42_KillerWhale",
                       "plots_w42", 
                       paste0("final_plot", ".png"))
  )

ggsave(
  filename = here::here("W42_KillerWhale",
                        "plots_w42", 
                        paste0("final_plot", ".png")),
  width    = 40, 
  height   = 30, 
