...
# ╔═╡ c7521067-c867-49e4-acd7-ac990c304ae3
begin
	using PlutoUI
	TableOfContents()
end

# ╔═╡ 27fab7c5-e5c6-4d6d-a170-b175699648f9
using Tidier, Chain

# ╔═╡ 631f9c15-7894-4115-b2b1-0323a11438e9
using Statistics

# ╔═╡ 8b18a432-5f7b-11ef-19c9-478210b1b08a
md"
# OLYMPICS GAMES
### TidyTuesday   Week 32
"

# ╔═╡ 5724940b-4829-4f8d-bdd6-b212c1702c53
md"---"

# ╔═╡ 0a72f2d2-3877-48d4-8f4a-ee76ac74422e
md"## Setup"

# ╔═╡ 6b48fe7b-b4a8-42b5-8751-91a5953063e6
md"### Packages"

# ╔═╡ 44505810-ed15-4b48-9621-b214452cf72c
import Makie.IntervalsBetween, Makie.Attributes

# ╔═╡ 51ea9cd1-f278-4e7b-b76c-b9efb5980dcb
md"### Data"

# ╔═╡ 2ebc1d03-0d05-40db-bb42-6216a0c41243
begin
url_main = "https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/"
url_data = "data/2024/2024-08-06/olympics.csv"
rdt = read_csv(url_main * url_data)
end;

# ╔═╡ 0e27b550-dbb3-4225-93ef-e9a5e13fc541
first(rdt, 3)

# ╔═╡ c834edd4-508f-4212-b924-15f8de63fc51
md"## Data Wrangling"

# ╔═╡ 99077c0b-044e-44d8-87be-9fce7d2e022b
df = @chain rdt begin
	@mutate(age = as_integer(age))
	@mutate(height = as_integer(height))
	@mutate(weight = as_integer(weight))
	@drop_missing()
end

# ╔═╡ 496778f9-64bc-457e-bf46-b17f77bb1c4c
@chain rdt begin
	@mutate(
		weight = as_integer(weight)
	)
end

# ╔═╡ 18cf2f98-2145-48fe-8673-8af6e342ac33
md"## EDA"

# ╔═╡ a1115d93-9317-4800-8a3d-95e54adb8a90
teams  = @chain rdt begin
    @count(team)
    @arrange(desc(n))
	@slice_head(n = 10)
	@pull(team)
...
# ╔═╡ b291b27a-8299-4793-920e-5e5c3984917d
top_countries = [
	"United States", "France", "Great Britain", "Italy", "Germany", 
	"Canada", "Japan", "Sweden", "Australia", "Hungary"]

# ╔═╡ 4985944c-61f0-40da-ba7b-3edecac45e45
md"### Top 20 Games by participants"

# ╔═╡ 720fd393-d784-4a60-83d6-8e3796213dab
@chain rdt begin
    @count(sport)
    @arrange(desc(n))
	@slice_head(n = 20)
    ggplot(xticklabelrotation = .5)
        geom_col(aes(y = :n, x = cat_inorder(:sport)))
end

# ╔═╡ 9554ad01-7913-4c7b-aff2-c82ff0e3390c
md"### Age, weight, height by gender and season"

# ╔═╡ 6f9a5a74-e502-4976-9d7e-3ed45e786dc9
beautiful_makie_theme = Attributes(
    fonts=(;regular="CMU Serif"),
)

# ╔═╡ 6741c888-6cf1-4930-a95d-d5ededa26184
robo_theme = Attributes(
    fonts=(;regular="Roboto"),
)

# ╔═╡ e2c3f230-61dc-49e0-9c8f-6c08799a14d2
@chain rdt begin
	@filter(team ∈ ["United States", "France", "Great Britain", 
					"Italy", "Germany", "Canada", "Japan", "Sweden", 
					"Australia", "Hungary"])
	
end

# ╔═╡ c42a8277-be78-45aa-913f-33dc748920e1
ggplot(rdt) + 
    geom_point(aes(x = :weight, y = :height))

# ╔═╡ 1521e587-ff77-4b64-8aa5-6e3453384878
ggplot(df) + 
    geom_point(aes(x = :weight, y = :height), alpha = .5)

# ╔═╡ 47d53225-e910-4b56-b07b-8076d82a97f6
@chain df begin
	ggplot() 
    geom_point(aes(x = :weight, y = :height), alpha = .5) 
	labs(
		...
		subtitle = "Relationship between age and height") 
	theme(
        xgridstyle=:dash,
        ygridstyle=:dash) +
...
# ╔═╡ caf5c931-c3d9-4d50-b976-6c82a4d4bf43
@chain df begin
	ggplot() 
    geom_point(aes(x = :weight, y = :height, color = :sex), alpha = .5) 
	labs(
		x = "Weight (Kg)", 
		y = "Height (cm)", 
		title = "Olympics Athletes:",
...
# ╔═╡ f8684a51-f081-4741-a5b4-9f5548a4c0c6
md"### Extension with Makie"

# ╔═╡ 19b0d48d-129b-4c3d-8748-713559ffb3af
geom_raincloud = geom_template("geom_raincloud", ["x", "y"], :RainClouds)

# ╔═╡ cf86b419-8caf-4e93-9515-5df34d2d6256
@chain df begin
	@filter(team ∈ ["United States", "China", "Kenya", "France"])
	ggplot() 
    geom_raincloud(
		aes(x = :team, y = :height, color = :team), 
		size = 4) 
    scale_y_continuous(labels = "{:.1f} cm") + 
    labs(
		title = "Olympics Athletes", 
		subtitle = "Height distribution by country",
		x = "Countries", y = "Height") +
    theme_minimal()
end
...
manifest_format = "2.0"
project_hash = "0dfbb86019138071ee35b1b403f1eba4672c369f"
...
git-tree-sha1 = "641122f4005e7f8c29b890dee2fc24c139f2f556"
...
git-tree-sha1 = "0e70b094d9981c7e0da45a4ff120b48ca9871125"
...
git-tree-sha1 = "eaf5c5c38401f2ab2f14a1f863ec13d12c6adfbb"
...
git-tree-sha1 = "d6860a6a03364bc5ecb4cb92935703336b1b5d98"
uuid = "337ecbd1-5042-4e2a-ae6f-ca776f97570a"
version = "0.7.7"

[[deps.TidierStrings]]
deps = ["StringEncodings"]
git-tree-sha1 = "eb0691f3ef8a60e4b49f13ad3b9f1e9be8e2f863"
